...
func _update_tile_immediate(tm: TileMapLayer, coord: Vector2i, ts_meta: Dictionary, types: Dictionary, cache: Array, specified_types: Array[int]) -> void:
	var type = types[coord]
...
func update_terrain_cells(tm: TileMapLayer, cells: Array, and_surrounding_cells := true, specified_types: Array[int] = []) -> void:
	if !tm or !tm.tile_set:
		return
	...
	var cache := _get_cache(tm.tile_set)
	for c in cells:
		_update_tile_immediate(tm, c, ts_meta, types, cache, specified_types)
...
func update_terrain_cell(tm: TileMapLayer, cell: Vector2i, and_surrounding_cells := true, specified_types: Array[int] = []) -> void:
	update_terrain_cells(tm, [cell], and_surrounding_cells, specified_types)
...
func update_terrain_area(tm: TileMapLayer, area: Rect2i, and_surrounding_cells := true, specified_types: Array[int] = []) -> void:
	if !tm or !tm.tile_set:
		return
	...
	for y in range(area.position.y, area.end.y):
		for x in range(area.position.x, area.end.x):
			var coord := Vector2i(x, y)
			_update_tile_immediate(tm, coord, ts_meta, types, cache, specified_types)
